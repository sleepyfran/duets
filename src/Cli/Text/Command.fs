...
let directionDescription direction =
    $"Follows the {Generic.directionName direction} direction"
...
let lookEntrances entrances =
    $"""You can go to {Generic.listOf entrances (fun (direction, name) ->
                           $"{Styles.place name} through the {Generic.directionName direction} ({Styles.information (Generic.directionCommand direction)})")}"""
