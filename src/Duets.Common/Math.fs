...
/// Rounds a decimal to its nearest int.
let roundDecimalToNearest (dec: decimal) = System.Math.Round(dec) |> int
...
/// Floors a float to its nearest integer value.
let floorToNearest (flt: float) = floor flt |> roundToNearest
