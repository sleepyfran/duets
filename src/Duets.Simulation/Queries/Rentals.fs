...
module Rentals =
    ...
    /// Returns a list of all upcoming seasonal rental payments within this week.
    let allUpcoming state =
        ...
        |> List.filter (fun rental ->
            ...
            | Seasonal nextPaymentDate -> nextPaymentDate <= nextWeekDate
            | _ -> false)
    ...
    let calculateSeasonalRentalPrice cityId (place: Place) =
