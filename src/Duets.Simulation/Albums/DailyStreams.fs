module Duets.Simulation.Albums.DailyStreams
...
let private calculateNonFanStreams state (band: Band) album genreMarket =
    ...
    let amountCap = Config.MusicSimulation.baseNonFanStreamCap / capModifier
    ...
    *)
    genreMarket * (albumQuality / amountCap) |> Math.ceil
...
let private calculateQualityModifier album =
    album |> Queries.Albums.quality |> (fun quality -> (float quality / 100.0))
...
let private calculateMaxDailyStreams state releasedAlbum =
    ...
    let fanStreams = calculateFanStreams band |> applyStreamModifiers album
...
let dailyStreams state releasedAlbum =
    ...
    (fanStreams * releasedAlbum.Hype |> Math.roundToNearest,
     nonFanStreams * releasedAlbum.Hype |> Math.ceilToNearest)
