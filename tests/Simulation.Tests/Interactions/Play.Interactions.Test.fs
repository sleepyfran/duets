...
let ``playing with a video-game console returns a video-game result`` () =
    ...
    |> List.filter (function
        | PlayedGame(PlayResult.VideoGame) -> true
        | _ -> false)
    |> should haveLength 1
...
let ``playing in a dartboard has a 50% chance of returning a winning dart result``
    ...
    |> Result.unwrap
    |> List.filter (function
        | PlayedGame(PlayResult.Darts(SimpleResult.Win)) -> true
        | _ -> false)
    |> should haveLength 1
...
let ``playing in a dartboard has a 50% chance of returning a losing dart result``
    ...
    |> Result.unwrap
    |> List.filter (function
        | PlayedGame(PlayResult.Darts(SimpleResult.Lose)) -> true
        | _ -> false)
    |> should haveLength 1
...
let ``playing in a billiard has a 50% chance of returning a winning pool result``
    ...
    |> Result.unwrap
    |> List.filter (function
        | PlayedGame(PlayResult.Pool(SimpleResult.Win)) -> true
        | _ -> false)
    |> should haveLength 1
...
let ``playing in a billiard has a 50% chance of returning a losing pool result``
    ...
    |> Result.unwrap
    |> List.filter (function
        | PlayedGame(PlayResult.Pool(SimpleResult.Lose)) -> true
        | _ -> false)
    |> should haveLength 1
