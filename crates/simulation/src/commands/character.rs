...
pub trait CharacterCommands {
    ...
    /// Increases mood by the specified amount if possible (< 100)
    fn increase_mood_by(self, amount: u8) -> Character;
    /// Decreases mood by the specified amount if possible (< 100)
    fn decrease_mood_by(self, amount: u8) -> Character;
}
...
impl CharacterCommands for Character {
    ...
    fn increase_mood_by(self, amount: u8) -> Character {
        let mood = &self.mood.saturating_add(amount);
        self.with_mood(*mood)
    }
    fn decrease_mood_by(self, amount: u8) -> Character {
        let mood = &self.mood.saturating_sub(amount);
        self.with_mood(*mood)
    }
}
